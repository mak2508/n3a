...
export function Meetings() {
  ...
  const handleFileUpload = async (file: File, meetingId: string) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/meetings/${meetingId}/upload-audio`, {
        method: 'POST',
        body: formData,
      });
      ...
      setUploadStatus(prev => ({ ...prev, progress: 30, currentStep: 'transcribing' }));
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      setUploadStatus(prev => ({ ...prev, progress: 50, currentStep: 'analyzing' }));
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      setUploadStatus(prev => ({ ...prev, progress: 70, currentStep: 'summarizing' }));
      await new Promise(resolve => setTimeout(resolve, 2000));
          ...
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="mt-6 space-y-6">
                ...
                {selectedMeeting && !selectedMeeting.audio_url && (
                  <div>
                    ...
                    <div className="mt-2 space-y-4">
                      <div className="flex items-center justify-center w-full">
                        <label 
                          ...
                          className={`w-full flex flex-col items-center justify-center px-4 py-6 bg-white text-gray-500 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-50 ${uploadStatus.isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                          <svg className="w-8 h-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                          </svg>
                          <span className="text-sm">Click to upload audio or drag and drop</span>
                          <span className="text-xs text-gray-400 mt-1">MP3, WAV, or M4A up to 50MB</span>
                          <input
                            id="audio-upload"
                            type="file"
                            className="hidden"
                            ...
                            onChange={async (e) => {
                              ...
                              await handleFileUpload(file, selectedMeeting.id);
                            }}
                          />
                        </label>
                      </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center text-sm text-gray-600">
                              ...
                              {uploadStatus.currentStep === 'uploading' && 'Uploading audio...'}
                              {uploadStatus.currentStep === 'transcribing' && 'Transcribing audio...'}
                              {uploadStatus.currentStep === 'analyzing' && 'Analyzing sentiment...'}
                              {uploadStatus.currentStep === 'summarizing' && 'Generating summary...'}
                              {uploadStatus.currentStep === 'completed' && 'Processing complete!'}
                            </div>
                            <ul className="space-y-1 text-sm text-gray-500">
                              <li className="flex items-center">
                                <svg className={`w-4 h-4 mr-2 ${uploadStatus.progress >= 30 ? 'text-green-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <li className="flex items-center">
                                <svg className={`w-4 h-4 mr-2 ${uploadStatus.currentStep === 'transcribing' ? 'text-blue-500 animate-spin' : uploadStatus.progress >= 50 ? 'text-green-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <li className="flex items-center">
                                <svg className={`w-4 h-4 mr-2 ${uploadStatus.currentStep === 'analyzing' ? 'text-blue-500 animate-spin' : uploadStatus.progress >= 70 ? 'text-green-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              ...
                              <li className="flex items-center">
                                <svg className={`w-4 h-4 mr-2 ${uploadStatus.currentStep === 'summarizing' ? 'text-blue-500 animate-spin' : uploadStatus.progress >= 90 ? 'text-green-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
